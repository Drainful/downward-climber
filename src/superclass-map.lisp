...
(defun inner (superclass-map)
  ...
  (declare (type superclass-map superclass-map))
  (superclass-map-inner-hash-map superclass-map))

(defun add-object (object superclass-map)
  ;; class must be equalp-able because it is inserted into a hash set
  "Add OBJECT to the superclass map given by SUPERCLASS MAP"
  (declare (type superclass-map superclass-map))
...
(defun remove-object (object superclass-map)
  ...
  (declare (type superclass-map superclass-map))
  (dolist (superclass (get-superclasses object))
    (erase (get (class-name superclass) (inner superclass-map)) object)))
...
provided, and the intersection of the given sets is found."
  (declare (type list superclasses)
           (type superclass-map superclass-map))
  (reduce #'intersection
...
(defun update-superclass-map (class superclass-map)
  ...
  (let ((removed-objects nil)) ; create a list to contain removed objects
    ...
    (dofor (class-and-objects (inner superclass-map)) ; for each superclass
...
(defun print-superclass-map (superclass-map stream)
